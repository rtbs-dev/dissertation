...
{{< embed A-codefigs.qmd#fig-obs-set >}}
...
:::{#fig-collab-projection layout="[[1,1]]"}

{{< embed A-codefigs.qmd#fig-bipartite  >}}
...
- _I asked F and J, but J wanted to add B's expertise before writing one of the sections_
...
:::: {#fig-colleague-groundtruth layout="[[1,1]]" layout-valign='center'}

1. $G\rightarrow E,H,C$
2. $E\rightarrow A,F$
3. $I \rightarrow F,J\quad J\rightarrow B$
4. $D \rightarrow H\quad H\rightarrow E$

{{< embed A-codefigs.qmd#fig-colleague >}}

::::

Fundamentally, the network in [@fig-colleague-groundtruth] shows which relationships the authors _depend_ on to accomplish their publishing activity.
...
[Graphic, from messy to clean OR big bipartite projection hairball? ]
...
Network "backboning," as it has come to be called [CITE] tries to find 
Meanwhile, underlying networks of dependencies that _cause_ node activation patterns are almost always more sparse than these hairballs.Accessing the dependency _backbone_ in a principled way is difficult, but doing so in a rapid, scalable manner is critical for practitioners to be able to make use of it to trim their hairballs.  
...
To begin adressing these issues, we present a guide to current practices,pitfalls, and how common statistical tools apply to the network recovery problem: a _Practitioner's Guide to Network Recovery.
...
Clarifying what different versions of what "relation" means (whether proximity or incidence), since network structure is intended to encode such relations as mathematical objects, despite common ambiguities and confusion around what practitioners intend on communicating through them.
...
Next, building on a gap found in the first part, we present a novel method, _Forest Pursuit_, to extract depencency networks when we know a _spreading process_ causes node activation (e.g. paper co-authorship caused by collaboration requests).
...
Forest Pursuit in its simplest form scales linearly with the size of active-node sets, being trivially parallelizeable and streamable over dataset size, and agnostic to network size overall.
