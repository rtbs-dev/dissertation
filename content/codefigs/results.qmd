...
(tidy
    ...
    # .to_latex(hrules=True,column_format='lcccccc')
...
f,ax = plt.subplots(ncols=4, figsize=((8,2.5)), sharey=True)
for a in ax: 
    a.axvline(0.5, color='0.9')
compare_algs_single(tidy, ['FPi']+alg_order, ax=ax[1])
# ax[0].set_xlabel('E[MCC]');

compare_algs_single(tidy, ['FPi']+alg_order, metric='MCC-max', ax=ax[2])
compare_algs_single(tidy, ['FPi']+alg_order, metric='MCC-min', ax=ax[3])

# ax[1].set_xlabel('E[F-M]');

compare_algs_single(tidy, ['FPi']+alg_order, metric='APS', ax=ax[0])
# ax[2].set_xlabel('APS');
