...
def log_partial_residuals(df, algs, target, feats, fig=None):
    ...
    for s in sbplts:
        ...
        t = np.linspace(xmin, x[np.sqrt(x)<ys[1]][-1])
        tp = t[2]
        s.plot( t,np.sqrt(t), color='k', alpha=0.1)
        s.annotate("n½", xy=(tp, np.sqrt(tp)), color='0.8', 
                    bbox=dict(facecolor='white', linewidth=0, alpha=0.5))
        t = np.linspace(xmin, x[x<ys[1]][-1])
        tp = t[2]
        s.plot( t,t, color='k', alpha=0.1)
        s.annotate("n", xy=(tp,tp), color='0.8', 
                    bbox=dict(facecolor='white', linewidth=0, alpha=0.5))
        t = np.linspace(xmin, x[x**2<ys[1]][-1])
        tp = t[2]
        s.plot(t,t**2, color='k', alpha=0.1)
        s.annotate("n²", xy=(tp, tp**2), color='0.8', 
                    bbox=dict(facecolor='white', linewidth=0, alpha=0.5))
        t = np.linspace(xmin, x[x**3<ys[1]][-1])
        tp = t[2]
        s.plot( t,t**3, color='k', alpha=0.1)
        s.annotate(r"n³", xy=(tp, tp**3), color='0.8', 
                    bbox=dict(facecolor='white', linewidth=0, alpha=0.5))
        s.set_ylim(ys)
    sbplts[0].set(ylabel=f'{target} (residual)')
    handles, labels = sbplts[0].get_legend_handles_labels()
...
#| fig-cap: |
#| 
#|   Comparison of MENDR recovery scores: 
#|   **FP**: Forest Pursuit 
#|   **GL**: GLASSO
...
g = compare_algs(tidy, alg_order, graph_order, metric_order,aspect=.8)
# g.title('MENDR Recovery Scores by Graph Type')
# g.fig.suptitle('MENDR Recovery Scores by Graph Type')
```
...
f.suptitle('Partial Residuals (regression on E[MCC])')
...
(so.Plot(tidy, x='n-nodes',y='seconds',color='name')
 ...
 .add(so.Range(color='k'),so.Est(errorbar=("pi",75)), so.Dodge())
 .add(so.Dot(marker='o', pointsize=5, color='k'), so.Agg('median'), so.Dodge())
 .scale(
     x=(so.Nominal()
...
f.suptitle('Partial Residuals (regression on computation time)');
```
...
```{python}
# from mendr.metrics import Contingent
# from mendr.experiments import _metrics
# a = si[0]['ground']
...
pd.concat([
    ...
    int_tidy.groupby(['name','kind','metric'], observed=True)['score'].mean().unstack().loc[['FP', 'FPi', 'GL']],
    int_tidy.groupby(['name','kind'], observed=True)['MIR'].mean().to_frame().loc[['FP','FPi','GL']]

], axis=1).round(2).swaplevel().sort_index()
```
...
```{python}
compare_algs(int_tidy, ['FP','FPi','GL'], graph_order, metric_order)
# plt.tight_layout()
```

```{python}
compare_algs_single(int_tidy, ['FP','FPi','GL'])
...
plt.xlabel('change in score (E[MCC])');
...
pd.read_csv(datapath/'efm-fp-logodds.csv')
 .assign(kind=lambda df: df['dataset'].str[:2])
 .replace(
    {'kind':['TR','BL','SC']}, value={'kind':['Tree','Block','ScaleFree']}
 )
)
...
# sns.catplot()
f,ax = plt.subplots(nrows=2, sharex=True, height_ratios=(2,1))
sns.swarmplot(data=logits, y='kind', x='log-odds', orient='h', hue='kind', log_scale=True, ax=ax[0])
# datapath
sns.histplot(logits['log-odds'], log_scale=True, color='0.7')
sns.despine()
ax[1].set_xlabel('Log-Odds-Ratio')
f.suptitle('Logistic Regression Coef. (EFM - FP vs. Ground Truth)');
