...
```{python}
f=log_partial_residuals(df, ['FP','GL'], 'seconds', ['n-jumps', 'n-nodes','n-walks'])
...
pd.concat([
    tidy.groupby(['metric','name'], observed=True)['score'].mean().unstack()[['FP','FPi','GL']],#.round(2)
    # tidy.groupby('name', observed=True)['MIR'].mean().to_frame().T[['FP','FPi','GL']]
]).round(2)
```
...
```{python}
compare_algs(tidy, ['FP','FPi','GL'], graph_order, metric_order)
```
