...
# nx.connected.is_connected(G)
    ...
    labels=adjust_text(labels, ax=ax)[0]
    for t in labels:
        #manipulate indiviual text objects
        # print(t)
        t.set_path_effects([path_effects.Stroke(linewidth=1, foreground='white'),
                       path_effects.Normal()])
...
```{python}
(-np.log(pd.DataFrame(asc.ochiai(X, pseudocts=0.5), index=Xdf.columns, columns=Xdf.columns)).unstack().pipe(lambda s: s.loc[s<0]))
```
...
insax = ax.inset_axes([.05,.05,0.3,0.3])
...
ax = draw_graph_communities(G_ds,pos_cos,colors)
...
ax = draw_graph_communities(G_tr, pos_tree, colors)
...
ax = draw_graph_communities(G_gl,pos_gl,colors)
# plt.title(
...
ax = draw_graph_communities(G_fp,pos_fp,colors)
# plt.title(
