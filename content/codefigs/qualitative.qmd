...
insax = ax.inset_axes([-.1,-.1,0.3,0.3])
...
insax = ax.inset_axes([-.15,-.15,0.3,0.3])
...
G = nx.from_pandas_adjacency(pd.DataFrame(_sq(e_gl:=~min_connected_filter(glasso).mask), index=animals_list.values, columns=animals_list.values))
...
G = nx.from_pandas_adjacency(pd.DataFrame(_sq(~min_connected_filter(glasso).mask), index=animals_list.values, columns=animals_list.values))
#
pos = nx.kamada_kawai_layout(G, pos=pos_cos)
draw_graph_communities(G,pos, colors)
# plt.title(
