...
Let's use the notation from @eq-edge-vectors, such that each observation is implicitly represented by some subgraph $c$ of the complete graph on $n$ nodes.
We are limiting our possible observation vector to a set of all possible subgraphs in our desired graph class $\mathcal{C}$, such that the edge-induced subgraph $c=K_V[E_c]\in\mathcal{C}$ of the complete graph $K_V$, with $E_c$ being the edge-set of $c$.
Plainly, the edges that gave rise to the node activations in a given observation must belong to a graph that in turn belongs to our class.
Assuming we can define an associated measure $\mu_{\mathcal{C}}(c)$ on $\mathcal{C}$, we are able to define a probability distribution over graphs in the class.^[
 This is certainly not a trivial assumption, and might either be ill-defined or require techniques like the Gumbel trkick [CITE] to approximate, unless the partition function $\mathbb{Z}$ has a closed form, or $\mu$ is already a probability measure on some $\sigma$-algebra over $\mathcal{C}$.
...
$$
\begin{gathered}
p_{\mathcal{C}}(c) = \frac{\mu_{\mathcal{C}}(c)}{\mathbb{Z}}\\
\mathbb{Z} = \sum_{c\in\mathcal{C}} \mu_{\mathcal{C}}(c)
\end{gathered}
$$
...
If a set of nodes $V_i\subseteq V$ were observed in the $i$th observation, this assumption narrows the distribution to only the subgraphs $c=K_{V_i}[E_c]\in\mathcal{C}\equiv\mathcal{C}[V_i]$.

The can be represented instead using the vectorization in @eq-edge-vectors, so that 
$$\mathbf{r}_i|\mathbf{x}_i \sim p_{\mathcal{C}[x_i]} $$ 
