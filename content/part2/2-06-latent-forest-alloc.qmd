...
Finally, we use this model to continue where FP left off by alternating between estimation of $R$ an $B$, in a method we call_Expected Forest Maximization_(EFM).
...
This would be very similar to the way collapsed Gibbs sampling works for Latent Dirichlet Allocation[CITE], but with edges selected from a spanning forest distribution instead of "topics" from a multinomial.
...
> _Alternate embedding the node activations as edge activations and combining these under a desire path model to estimate the graph structure._ 
