...
The spanning tree distribution discussed elsewhere[@BayesianSpanningTree_Duan2021;@EfficientComputationExpectations_Zmigrod2021] can be used to motivate a spanning forest distribution, which is based on the Laplacian of the augmented graph $L^+$.^[Here, $L^+(G)$ and $L^+(G/e)$ represent the Laplacian of $G_{+R}$ and $G_{+R}$ without edge $e$, respectively.]
...
Using this equivalence to apply Kirchoff's matrix tree theorem, since we can calculate the probability of an edge being in a forest as the negation of the probability of a forest _without that edge_:
$$\begin{aligned}
...
To do inference, however, we may again use the forest kernel $Q$ and sum over the observed nodes to determine the likelihood of an observation $P(x_{ij}|z_E,\phi_{ij})$, which is just the probability of a node occurring on the same subtree as the marked node.
As previously discussed, this is just $Q(j,\phi)$!
Let's marginalize over the uniformly distributed $\phi$, for an overall likelihood of $\mathbf{x}_i$  
$$
