...
We have updated the original example with explicit partial orders in @fig-obs-cascade. Note that nodes in some lists could be re-arranged while keeping the partial order the same (keeping all arrows pointing to the right). 

{{< embed ../codefigs/graphs.qmd#fig-obs-cascade >}}
