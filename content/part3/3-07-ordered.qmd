...
To find out, we use softmax normalization
[^2]
...
Note that @eq-absorb is effectively the same as a ($\beta=1$) forest matrix if the transition matrix was replaced by a (sub-)graph laplacian.
...
@fig-fluency-dsmin looks very similar to the network recovered to make Fig. 4 in [@semanticorganizationanimal_Goni2011] 
