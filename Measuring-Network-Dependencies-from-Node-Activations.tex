...
``edge list'', which can be modeled with a network: a simple graph.
...
``Space''}\label{observation-and-feature-space}
...
are various tenchniques for dealign with e.g.~categorical data, which
include one-hot encoding (where \(k\) is the number of possible
categories, with boolean entries for each feature). A collection of
\(n\) data points, then, can be thought of as a matrix
\(X\in\mathbb{R}^{n\times k}\), such that each row is an observation and
each column reflects the location of each observation along that
feature's coordinate.

Rows and columns depend on domain\ldots we chose one. {[}FOOTNOTE{]}
