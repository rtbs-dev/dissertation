...
\section{Metrology as matrices}\label{metrology-as-matrices}
...
\subsection{Models \& linear operators}\label{models-linear-operators}
...
parameters from our observed data. If \(F\) is invertible, the most
...
the \emph{residual}:
\(\mathbf{p}^*=\min_{\mathbf{p}} F\mathbf{p}-\mathbf{x}\)
...
\chapter{Projections and Structure
Maps}\label{projections-and-structure-maps}
...
\chapter{Choosing a structure recovery
