...
\begin{tcolorbox}[enhanced jigsaw, opacityback=0, leftrule=.75mm, colback=white, rightrule=.15mm, bottomrule=.15mm, breakable, toprule=.15mm, arc=.35mm, left=2mm, colframe=quarto-callout-note-color-frame]
...
``unstructured'' data, which \textcite{tble-mwo} shows in comparison to
...
\tightlist
\item
  look, Equation~\ref{eq-trans-matrix}, Equation~\ref{eq-absorb} is the
  same as forest matrix.
\item
...
\caption{\label{fig-excavate-tern}Semisupervised MWO Classification with
...
\autocite{magicalnumberseven_Miller1956}
