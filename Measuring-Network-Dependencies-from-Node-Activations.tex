...
\tightlist
\item
  aggregate/marginalize over roots in a rooted tree (efficient
  expectations)\ldots also MI is symmetric (same tree for any root)
\item
  total tree: overcounting of independence is at most the spanning tree
  (hunter worsely)
\item
  generally max spanning tree is the mode for distribution, so there's
  our point estimate.
\end{itemize}
...
\tightlist
\item
  KL-divergence is convex (we can minimize sum of dists, not just dist
  of sum)
\item
  steiner tree and approx with closure\ldots which we don't have
\item
  oh look! counting trees is similar. use cosine because geometric
  pooling
\end{itemize}
