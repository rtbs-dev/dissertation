...
\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, toprule=.15mm, colframe=quarto-callout-note-color-frame, leftrule=.75mm, breakable, opacityback=0, bottomrule=.15mm, colback=white, rightrule=.15mm, left=2mm]
...
\chapter{Latent Graphs with Desire
Paths}\label{latent-graphs-with-desire-paths}
...
over subgraphs in the class.\footnote{ This is certainly not a trivial
  ...
  graphs{[}CITE{]}, or geometric spanners like Urqhart{[}CITE{]} and
  Relative Neighborhood {[}CITE{]} graphs on a ``sprinkling'' of points
  that preserves their observed pairwise distances.\\
...
\emph{Forest
Pursuit}}{Approximate Recovery in Near-linear Time by Forest Pursuit}}\label{approximate-recovery-in-near-linear-time-by-forest-pursuit}
...
Allocation{[}CITE{]}, but with edges selected from a spanning forest
distribution instead of ``topics'' from a multinomial. Derivations and
implementation of such a scheme is left for future work.
