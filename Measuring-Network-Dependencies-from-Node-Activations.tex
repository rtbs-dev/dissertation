...
boolean entries for each feature). A collection of \(n\) data points,
then, can be thought of as a matrix \(X\in\mathbb{R}^{n\times k}\), such
that each row is an observation and each column reflects the location of
each observation along that feature's coordinate.
