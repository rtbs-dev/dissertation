...
contemporaneously, or in some other context or artifact. These are
naturally represented as ``bipartite'' networks, having separate entites
for, say, ``papers'' and ``authors'', and connecting them with edges as
needed. But analysts are typically seeking the collaboration network
connecting authors, something not so directly observed, but which
\emph{if recovered} can provide estimates for community structure,
importance of individual authors as controling information flows, and
the ``distances'' that separate authors from each other, in their
respective domains. {[}CITE Newman{]}
...
\subcaption{\label{fig-bipartite}}
